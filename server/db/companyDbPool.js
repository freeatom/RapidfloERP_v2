// Company Database Pool Manager
// Manages separate SQLite databases per company for true data isolation
...
// Connection pool: companyId → { db, lastAccess }
const pool = new Map();
const IDLE_TIMEOUT = 5 * 60 * 1000; // 5 minutes

// Company database schema — ALL business tables
const COMPANY_SCHEMA = `
    -- Users & Roles for THIS company
    ...
    CREATE TABLE IF NOT EXISTS documents (
      ...
      size INTEGER DEFAULT 0, description TEXT DEFAULT '',
      uploaded_by TEXT REFERENCES users(id), is_deleted INTEGER DEFAULT 0,
...
// Default roles seeded into each new company DB
const SEED_ROLES = [
    { id: 'role-admin', name: 'admin', description: 'Company Administrator', level: 2, is_system: 1 },
    { id: 'role-manager', name: 'manager', description: 'Manager', level: 3, is_system: 1 },
    { id: 'role-user', name: 'user', description: 'Standard User', level: 4, is_system: 1 },
    { id: 'role-viewer', name: 'viewer', description: 'Read-only Access', level: 5, is_system: 1 },
];
...
const SEED_PERMISSIONS = [
    ...
    ['hrms', 'view'], ['hrms', 'create'], ['hrms', 'edit'], ['hrms', 'delete'], ['hrms', 'approve'],
...
function seedCompanyDb(db) {
    const insertRole = db.prepare('INSERT OR IGNORE INTO roles (id, name, description, level, is_system) VALUES (?,?,?,?,?)');
    for (const r of SEED_ROLES) insertRole.run(r.id, r.name, r.description, r.level, r.is_system);

    const insertPerm = db.prepare('INSERT OR IGNORE INTO permissions (id, module, action, description) VALUES (?,?,?,?)');
    for (const [mod, act] of SEED_PERMISSIONS) insertPerm.run(`perm-${mod}-${act}`, mod, act, `${act} ${mod}`);

    // Give admin all permissions
    const insertRolePerm = db.prepare('INSERT OR IGNORE INTO role_permissions (id, role_id, permission_id) VALUES (?,?,?)');
    for (const [mod, act] of SEED_PERMISSIONS) {
        insertRolePerm.run(`rp-admin-${mod}-${act}`, 'role-admin', `perm-${mod}-${act}`);
    }
    // Give manager most permissions (not admin/delete)
    for (const [mod, act] of SEED_PERMISSIONS) {
        if (act !== 'manage' && mod !== 'admin') {
            insertRolePerm.run(`rp-manager-${mod}-${act}`, 'role-manager', `perm-${mod}-${act}`);
        }
    }
    // Give user view/create/edit
    for (const [mod, act] of SEED_PERMISSIONS) {
        if (['view', 'create', 'edit'].includes(act) && mod !== 'admin') {
            insertRolePerm.run(`rp-user-${mod}-${act}`, 'role-user', `perm-${mod}-${act}`);
        }
    }
    // Give viewer only view
    for (const [mod, act] of SEED_PERMISSIONS) {
        if (act === 'view') {
            insertRolePerm.run(`rp-viewer-${mod}-${act}`, 'role-viewer', `perm-${mod}-${act}`);
        }
    }

    // Seed modules
    const insertModule = db.prepare('INSERT OR IGNORE INTO module_config (id, module, display_name, icon, sort_order, is_enabled) VALUES (?,?,?,?,?,1)');
    for (const m of SEED_MODULES) insertModule.run(`mod-${m.module}`, m.module, m.display_name, m.icon, m.sort_order);
}

/**
 * Get or create a company database connection
 */
export function getCompanyDb(companyId) {
    if (!companyId) return null;

    const cached = pool.get(companyId);
    if (cached) {
    ...
    if (!fs.existsSync(dbPath)) return null; // Company DB doesn't exist

    const db = new Database(dbPath);
    db.pragma('journal_mode = WAL');
    db.pragma('foreign_keys = ON');

    pool.set(companyId, { db, lastAccess: Date.now() });
    return db;
}

/**
 * Create a new company database with full schema and seed data
 */
export function createCompanyDb(companyId) {
    const dbPath = path.join(COMPANIES_DIR, `company_${companyId}.db`);
    ...
    db.exec(COMPANY_SCHEMA);
    db.exec(COMPANY_INDEXES);
    seedCompanyDb(db);

    pool.set(companyId, { db, lastAccess: Date.now() });
    console.log(`✅ Company database created: ${dbPath}`);
    return db;
}
 ...
 */
export function deleteCompanyDb(companyId) {
    const cached = pool.get(companyId);
    if (cached) {
        try { cached.db.close(); } catch (e) { /* ignore */ }
    ...
    try {
        ...
        console.log(`🗑️ Company database deleted: ${dbPath}`);
    } catch (e) {
        console.error('Error deleting company DB:', e.message);
    }
}

/**
 * List all company DB files
 */
export function listCompanyDbs() {
    if (!fs.existsSync(COMPANIES_DIR)) return [];
    return fs.readdirSync(COMPANIES_DIR)
        .filter(f => f.startsWith('company_') && f.endsWith('.db'))
        .map(f => f.replace('company_', '').replace('.db', ''));
}
