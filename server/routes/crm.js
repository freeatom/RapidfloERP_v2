...
function buildWhere(filters) {
    ...
    Object.entries(filters).forEach(([k, v]) => {
        if (v !== undefined && v !== '') { where.push(`${k} = ?`); params.push(v); }
    });
...
router.post('/leads', checkPermission('crm', 'create'), auditLog('crm', 'CREATE_LEAD'), (req, res) => {
    ...
    let scoreLabel = b.score_label || 'cold';
    const score = b.score || 0;
    if (score >= 80) scoreLabel = 'hot'; else if (score >= 50) scoreLabel = 'warm';
...
router.post('/leads/:id/convert', checkPermission('crm', 'create'), auditLog('crm', 'CONVERT_LEAD'), (req, res) => {
    ...
    db.prepare("UPDATE leads SET status='converted',converted_to_opportunity_id=?,converted_to_contact_id=?,converted_at=datetime('now'),updated_at=datetime('now') WHERE id=?").run(oppId, contactId, lead.id);
...
router.delete('/opportunities/:id', checkPermission('crm', 'delete'), auditLog('crm', 'DELETE_OPP'), (req, res) => {
    req.app.get('db').prepare('DELETE FROM opportunities WHERE id=?').run(req.params.id);
    res.json({ success: true });
});
...
router.get('/accounts', checkPermission('crm', 'view'), (req, res) => {
    ...
    const total = db.prepare(`SELECT COUNT(*) as count FROM accounts WHERE ${where.join(' AND ')}`).get(...params);
...
router.delete('/accounts/:id', checkPermission('crm', 'delete'), auditLog('crm', 'DELETE_ACCOUNT'), (req, res) => {
    req.app.get('db').prepare('DELETE FROM accounts WHERE id=?').run(req.params.id);
    res.json({ success: true });
});
...
router.delete('/contacts/:id', checkPermission('crm', 'delete'), auditLog('crm', 'DELETE_CONTACT'), (req, res) => {
    req.app.get('db').prepare('DELETE FROM contacts WHERE id=?').run(req.params.id);
    res.json({ success: true });
});
...
router.get('/activities', checkPermission('crm', 'view'), (req, res) => {
    ...
    const total = db.prepare(`SELECT COUNT(*) as count FROM activities WHERE ${where.join(' AND ')}`).get(...params);
...
router.delete('/activities/:id', checkPermission('crm', 'delete'), (req, res) => {
    req.app.get('db').prepare('DELETE FROM activities WHERE id=?').run(req.params.id);
    res.json({ success: true });
});
...
router.get('/stats', checkPermission('crm', 'view'), (req, res) => {
    ...
    res.json({ totalLeads, conversionRate, pipelineValue, hotLeads, wonThisMonth, totalAccounts });
});
